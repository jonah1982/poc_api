...
public class Tool {
	 ...
	 */
	public static void executeCommand(ApplicationContext context, String command) {
		...
		try {
			...
			String procOpLine=null;
			while((procOpLine=br.readLine())!=null){
				context.log(procOpLine);			
			}
			int exitValue=p.waitFor();
			context.log("Finished: "+exitValue);
		} catch (Exception e){
			context.log("execute command Error: "+e.getMessage());
			e.printStackTrace();
		}
	}
	...
	public static String addURLTime(String url, long timemilli) {
		...
		Long newsttimestring = Long.valueOf(sttimestring)+timemilli;
		...
		Long newettimestring = Long.valueOf(ettimestring)+timemilli;
	...
	public static Class<?> getViewTemplate(String s) throws Exception {
		return Class.forName(StringUtils.newStringUtf8(Base64.decodeBase64(s)));
	}
	...
	public static double getNiceDouble(double d) {
		return getNumDouble(d, 2);
	}
	...
	public static boolean containKeywordsOr(String content, String keywords, String delimiter) {
		StringTokenizer st = new StringTokenizer(keywords, delimiter+" ");
		while(st.hasMoreTokens()) {
			String kw = st.nextToken();
			if(content.toLowerCase().contains(kw.toLowerCase())) return true;
		}
		return false;
	}
